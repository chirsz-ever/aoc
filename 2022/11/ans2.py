...
common_factor = prod(mk.test_factor for mk in monkeys)
...
for r in range(10000):
    for i, mk in enumerate(monkeys):
        ...
        for it in mk.items:
            ...
            target_id = mk.on_test_true if divisible else mk.on_test_false
            target = monkeys[target_id]
            target.items.append(lvl % common_factor)
        inspect_stat[i] += len(mk.items)
        mk.items.clear()
