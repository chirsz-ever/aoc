...
def main() -> None:
    ...
    queue: deque[tuple[int, int, tuple[int, int]]] = deque([(0, 0, vmap0.start)])
    visited: set[tuple[int, int, tuple[int, int]]] = set()
    status = 0
    while len(queue) > 0 and status != 3:
        t, s, (x, y) = queue.popleft()
        ...
        for nx, ny in next_pos(m1, x, y):
            s1 = s
            if s == 0 and (nx, ny) == m1.target:
                s1 = 1
                # print(f"{s1}: {t + 1}")
            elif s == 1 and (nx, ny) == m1.start:
                s1 = 2
                # print(f"{s1}: {t + 1}")
            elif s == 2 and (nx, ny) == m1.target:
                s1 = 3
                print(f"{s1}: {t + 1}")
                exit(0)
            ...
            #     continue
            if (t + 1, s1, (nx, ny)) not in visited:
                visited.add((t + 1, s1, (nx, ny)))
                queue.append((t + 1, s1, (nx, ny)))
