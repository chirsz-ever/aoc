...
fn step_grid(grids: &[Vec<Grid>], row: usize, col: usize) -> Grid {
    match (grids[row][col], count_see(grids, row, col)) {
        (Empty, 0) => Occupied,
        (Occupied, n) if n >= 5 => Empty,
        (g, _) => g,
    }
}

fn count_see(grids: &[Vec<Grid>], row: usize, col: usize) -> usize {
    let row = row as i32;
    let col = col as i32;
    let rows = grids.len() as i32;
    let cols = grids[0].len() as i32;
    let count_by = |&(dr, dc)| -> usize {
        let (mut r, mut c) = (row, col);
        r += dr;
        c += dc;
        while (0..rows).contains(&r) && (0..cols).contains(&c) {
            //dbg!((c,r));
            match grids[r as usize][c as usize] {
                Occupied => return 1,
                Empty => return 0,
                Floor => {},
            }
